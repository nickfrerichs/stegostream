...
class PeerConnection:
    ...
    def __processSendBuffer(self):
        while True:
            ...
            with self.send_buffer.lock:
                for channel, buffer in self.send_buffer.var.items():
                    try:
                        (message, seq, last) = buffer.queue.get_nowait()
                        self.__send_data(message, channel)
                    except queue.Empty:
                        pass
    ...
    def send_text(self, text):
        ...
        for index in range(msg.chunk_count):
            ...
            last = index == msg.chunk_count - 1
            if last:
                self.__send_data(chunk, 1, Flags.set(Flags.END_DATA))
            else:
                self.__send_data(chunk, 1)



    def __send_data(self, binary_data, channel, bin_flag = None):
        ...
        # Store to buffer in case it needs to be re-sent
        with self.send_buffer.lock, self.seq.lock:
            self.seq.var[1] += 1
            self.send_buffer.var[1].add_message(binary_data, self.seq.var[1], bin_flag)
        ...
        if channel == 0:
            data = channel.to_bytes(1, byteorder='big')+bin_flag + channel.to_bytes(2, byteorder='big') + channel.to_bytes(2, byteorder='big') + binary_data
        elif channel > 0:
            with self.seq.lock, self.ack.lock:
                # self.seq.var[channel] += 1
                data = channel.to_bytes(1, byteorder='big')+bin_flag + self.ack.var[channel].to_bytes(2, byteorder='big') + self.seq.var[channel].to_bytes(2, byteorder='big') + binary_data
